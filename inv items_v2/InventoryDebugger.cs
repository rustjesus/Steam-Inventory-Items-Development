...
public class InventoryDebugger : MonoBehaviour
{
    ...
    public void RefreshStacks()
    {
        ...
        foreach (var kvp in stacks)
        {
            int id = kvp.Key;
            int qty = kvp.Value;
            int[] sizes = InventoryManager.GetStackSizes(id);

            // Fallback: ensure a single item always has a valid stack array
            if ((sizes == null || sizes.Length == 0) && qty > 0)
            {
                sizes = new int[] { qty };
                InventoryManager.SetStacks(id, sizes);
            }

            debugStacks.Add(new ItemStackEntry
            {
                itemId = id,
                //quantity = qty, removed to fix count bug
                quantity = ObscuredPrefs.GetInt("Item" + id),
                stackCount = sizes.Length,
                stackSizes = sizes
            });
        }
    }
    ...
    private void UpdateUIText()
    {
        ...
        int qty = InventoryManager.GetQuantity(currentItemDefToStack);
        int count = InventoryManager.GetStackCount(currentItemDefToStack);
        int[] sizes = InventoryManager.GetStackSizes(currentItemDefToStack);
    ...
    private void ConfirmMakeStacksOf()
    {
        ...
        int itemDefId = currentItemDefToStack;
